...
function eventStats(eventId) {
  const rows = db.prepare('SELECT rsvp, COUNT(*) c, SUM(party_size) ps FROM guests WHERE event_id = ? GROUP BY rsvp')
    .all(eventId);
  const stats = { yes: 0, no: 0, maybe: 0, pending: 0, headcount: 0, total: 0 };
  for (const r of rows) {
    if (stats[r.rsvp] != null) stats[r.rsvp] = r.c;
    stats.total += r.c;
    if (r.rsvp === 'yes') stats.headcount += r.ps || 0;
  }
  return stats;
}
